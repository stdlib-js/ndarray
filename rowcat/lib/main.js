...
var prependSingletonDimensions = require( './../../base/prepend-singleton-dimensions' );
...
* @throws {Error} must provide ndarrays having the same number of columns
...
function rowcat( arrays ) {
	...
	for ( i = 0; i < arrays.length; i++ ) {
		...
		if ( n === 1 ) {
			arr = prependSingletonDimensions( ndarraylike2ndarray( arr ), 1, false );
		}
		sh = getShape( arr, false );
		if ( i === 0 ) {
			ncols = sh[ 1 ];
		} else if ( sh[ 1 ] !== ncols ) {
			throw new Error( format( 'invalid argument. All ndarrays must have the same number of columns. Expected number of columns: `%u`. Actual number of columns: `%u`.', ncols, sh[ 1 ] ) );
		}
		arrs.push( arr );
	}
	return concat( arrs, {
