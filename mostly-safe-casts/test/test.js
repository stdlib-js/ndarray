...
tape( 'main export is a function', function test( t ) {
	...
	t.strictEqual( typeof mostlySafeCasts, 'function', 'main export is a function' );
	t.end();
});
...
tape( 'the function returns a list of ndarray data types to which a provided ndarray data type can be cast', function test( t ) {
	var list;
	var i;
...
tape( 'if provided an unrecognized or unsupported data type, the function returns `null`', function test( t ) {
	...
	for ( i = 0; i < values.length; i++ ) {
		t.strictEqual( mostlySafeCasts( values[ i ] ), null, 'returns expected value when provided '+values[ i ] );
	}
	t.end();
});
