...
// Logger:
var debug = logger( 'ndarray-count-falsy-inline-loops:script' );
...
var INCLUDE_DIR = path.resolve( BASE_INCLUDE_DIR, 'count-falsy' );
var SRC_DIR = path.resolve( __dirname, '..', 'src', 'loops' );

// Main header file:
var INCLUDE_MAIN = path.join( BASE_INCLUDE_DIR, 'count_falsy.h' );
...
function updateMainHeader( signatures ) {
	...
	for ( i = 0; i < signatures.length; i++ ) {
		...
		list.push( '#include "count-falsy/loops/'+sig+'.h"' );
	}
	file = [
