...
var base = require( './../../base/every-by' );
...
*/
function everyBy( x, options, predicate, thisArg ) {
	var nargs;
	var opts;
	var view;
	var ctx;
	...
	// Case: everyBy( x, options, predicate, thisArg )
	else if ( nargs > 3 ) {
		flg = true;
		o = options;
		cb = predicate;
	...
	// Case: everyBy( x, predicate, thisArg )
	else if ( isFunction( options ) ) {
		cb = options;
		ctx = predicate;
	}
	// Case: everyBy( x, options, predicate )
